	...
	<div slot="content">
		...
		>
			...
			>
				...
				{:else}
					<BookmarkIcon class="size-4" strokeWidth="2" />
			...
			>
				<PencilIcon class="size-4" strokeWidth="2" />
			...
			>
				<ArchiveIcon class="size-4" strokeWidth="2" />
			...
			<DropdownMenu.Sub>
				<DropdownMenu.SubTrigger
					class="flex gap-2 items-center px-3 py-2 text-sm  cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl text-gray-600 hover:text-black"
				>
					<DownloadIcon class="size-4" strokeWidth="2" />
				...
				<DropdownMenu.SubContent
					class="w-[180px] rounded-2xl p-1 border border-gray-300/30 dark:border-gray-700/50 z-50 bg-white dark:bg-gray-850 dark:text-white shadow-sm flex flex-col gap-1"
					transition={flyAndScale}
					sideOffset={10}
				>
					{#if $user?.role === 'admin' || ($user.permissions?.chat?.export ?? true)}
						...
						>
							<div class="flex items-center line-clamp-1">{$i18n.t('Файл JSON')}</div>
						</DropdownMenu.Item>
					{/if}
					...
					>
						<div class="flex items-center line-clamp-1">{$i18n.t('Текстовый документ')}</div>
					</DropdownMenu.Item>
					...
					>
						<div class="flex items-center line-clamp-1">{$i18n.t('Файл PDF')}</div>
					</DropdownMenu.Item>
				</DropdownMenu.SubContent>
			</DropdownMenu.Sub>
			<DropdownMenu.Item
			...
			>
				<Trash class="size-4" strokeWidth="2" />
