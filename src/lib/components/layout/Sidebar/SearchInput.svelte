<script lang="ts">
	...
	let options = [
		{
			name: 'Теги:',
			description: $i18n.t('поиск по тегам')
		}
	];
	let focused = false;
	let loading = false;
...
<div class="px-1 flex h-full border border-gray-200 bg-white rounded-[15px] relative z-20" id="search-container">
	<div class="flex w-full rounded-[15px]" id="chat-search">
		<div class="self-center text-neutral-400 pl-2 py-2 rounded-l-[15px] bg-transparent">
			<svg
		...
		<input
			class="w-full rounded-r-[15px] py-1.5 pl-2 text-sm bg-transparent dark:text-gray-300 outline-hidden"
	...
	{#if focused && (filteredOptions.length > 0 || filteredTags.length > 0)}
		...
		<div
			class="absolute top-0 mt-10 left-0 right-1 border border-gray-100 dark:border-gray-900 bg-white dark:bg-gray-950 rounded-xl shadow-xs"
			in:fade={{ duration: 50 }}
			on:mouseenter={() => {
				selectedIdx = null;
			}}
		...
		>
			<div class="px-2 py-2 text-xs group flex flex-col gap-1">
				{#if filteredTags.length > 0}
					<div class="px-2.5 dark:text-gray-300 text-gray-600 mb-1">Найденные теги</div>

					<div class="max-h-60 overflow-auto flex flex-col gap-0.5 scrollbar-none">
						{#each filteredTags as tag, tagIdx}
							<button
								class=" px-2.5 py-1.5 flex gap-1 hover:bg-gray-100 dark:hover:bg-gray-900 w-full rounded-lg {selectedIdx ===
								tagIdx
				...
				{:else if filteredOptions.length > 0}
					<div class="px-2.5 dark:text-gray-300 text-gray-600 mb-1">
						{$i18n.t('Переметры поиска')}
					</div>

					<div class=" max-h-60 overflow-auto">
						{#each filteredOptions as option, optionIdx}
							<button
								class=" px-2.5 py-1 flex gap-1 bg-gray-50 hover:bg-gray-100 dark:hover:bg-gray-900 w-full rounded-lg {selectedIdx ===
								optionIdx
