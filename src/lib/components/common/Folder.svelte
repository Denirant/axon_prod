<script lang="ts">
	...
	import { BookmarkIcon, ChevronDownIcon } from 'lucide-svelte';
...
<div bind:this={folderElement} class="relative {className}">
	...
	{#if collapsible}
		...
		>
			...
			<span
				class="w-full group bg-transparent hover:bg-neutral-200/70 cursor-pointer relative flex items-center gap-1.5 mt-1 rounded-[10px] px-1.5 py-1.5 text-sm text-neutral-600 dark:text-gray-500 font-medium"
			>
				<BookmarkIcon class="size-4 text-yellow-400 fill-yellow-400 opacity-80" />
				{name}

				<span
					class="flex flex-1 absolute top-1/2 -translate-y-1/2 right-3 w-4 h-4 text-neutral-500/80 group-hover:text-neutral-700"
				>
					<ChevronDownIcon
						class={`size-4 transition-all duration-200 ${open ? 'rotate-180' : 'rotate-0'}`}
					/>
				</span>
			</span>
