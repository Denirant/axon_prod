	...
	>
		...
		<div class="flex-auto w-0 pl-1">
			<Name>
				...
				<span
					class="opacity-0 group-hover:opacity-100 w-[1px] bg-gray-100 h-4 mx-2 block transition"
				></span>

				{#if message.timestamp}
					<div
						class="self-center select-none text-[10px] opacity-0 group-hover:opacity-100 text-gray-300 font-normal transition first-letter:capitalize ml-0.5 translate-y-[1px]"
					>
					...
					<div>
						{#if (message?.statusHistory ?? [...(message?.status ? [message?.status] : [])]).length > 0}
							...
							{#if !status?.hidden}
								...
								{#if status?.description === 'Generating an image'}
								{#if status?.description === 'Generating an image'}
								<div class="w-120 h-120 rounded-2xl overflow-hidden mt-1 border border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-800/70">
									<!-- Упрощенный скелетон с анимацией пульсации, полностью на Tailwind CSS -->
									<div class="relative w-full h-full flex flex-col items-center justify-center">
										<!-- Фоновая пульсация с помощью Tailwind -->
										<div class="absolute inset-0 bg-gradient-to-r from-transparent via-white to-transparent dark:via-gray-700 animate-pulse"></div>
										
										<!-- Иконка загрузки из Lucide с анимацией вращения -->
										<Loader2 class="w-8 h-8 text-gray-400 dark:text-gray-500 animate-spin mb-2" />
...
<style>
	...
	.skeleton-loader {
		position: relative;
		overflow: hidden;
		background-color: #f9fafb; /* Светлый нейтральный фон для светлой темы */
		border-radius: 1rem;
	}

	.dark .skeleton-loader {
		background-color: #1f2937; /* Тёмный фон для тёмной темы */
	}

	/* Основной переливающийся эффект */
	...
	/* Улучшенная волнистая анимация - более плавная и быстрая */
	.skeleton-wave {
		position: relative;
	}
