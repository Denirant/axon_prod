<script lang="ts">
	...
	import { ArrowDownToLine, Check, Clipboard, ClipboardCheck, DownloadIcon } from 'lucide-svelte';
...
<div>
	<div class="relative {className} flex flex-col rounded-lg" dir="ltr">
		{#if lang === 'mermaid'}
			...
			{/if}
		{:else}
			<div class="text-text-300 absolute mt-2 pl-4 text-xs font-medium dark:text-white">
				{lang}
			</div>

			<div
				class="sticky {stickyButtonsClassName} pt-1 pb-1.5 pr-2.5 flex items-center justify-end z-10 text-xs text-black dark:text-white"
			>
				<div class="flex items-center gap-1 translate-y-[1px]">
					<button
						class="flex gap-1 items-center bg-none border-none bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 transition rounded-lg px-2 py-1"
					...
					>
						...
						<div>
							{collapsed ? $i18n.t('Развернуть') : $i18n.t('Collapse')}
						</div>
					</button>
					...
					{#if ($config?.features?.enable_code_execution ?? true) && (lang.toLowerCase() === 'python' || lang.toLowerCase() === 'py' || (lang === '' && checkPythonCode(code)))}
						...
						{:else if run}
							<button
								class="flex gap-1 items-center run-code-button bg-none border-none bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 transition rounded-lg px-2 py-1"
								on:click={async () => {
									code = _code;
									await tick();
									executePython(code);
					...
					{#if save}
						<button
							class="save-code-button flex gap-1 items-center bg-none border-none bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 transition rounded-lg px-2 py-1"
							on:click={saveCode}
						>
							{#if !saved}
					...
					<button
						class="copy-code-button flex gap-1 items-center bg-none border-none bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 transition rounded-lg px-2 py-1"
						on:click={copyCode}
					>
						{#if !copied}
