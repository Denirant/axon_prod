<script lang="ts">
	...
	const askHandler = async () => {
		...
		prompt = `${floatingInputValue}\n\`\`\`\n${selectedText}\n\`\`\`.Provide as short but comprehensive an explanation as possible. Answer in the language of the original highlighted text.`;
	...
	const explainHandler = async () => {
		...
		floatingInputValue = `Объясни: \`${selectedText}\``
		prompt = `${'Provide as short but comprehensive an explanation as possible. Answer in the language of the original highlighted text.'}\n\n\`\`\`\n${selectedText}\n\`\`\``;
		...
		<div class="bg-white dark:bg-gray-850 dark:text-gray-100 rounded-xl shadow-xl w-80 max-w-full">
			...
			>
				<div class="font-medium">
					<Markdown id={`${id}-float-prompt`} content={floatingInputValue} />
				</div>
			</div>
